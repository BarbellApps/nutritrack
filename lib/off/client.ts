...
export async function lookupBarcode(barcode: string): Promise<OFFProduct | null> {
  const res = await fetch(
    `${OFF_BASE}/api/v2/product/${encodeURIComponent(barcode)}.json?fields=product_name,brands,serving_quantity,serving_size,nutriments`,
...
export async function searchOFF(query: string, limit = 15): Promise<OFFProduct[]> {
  const res = await fetch(
    `${OFF_BASE}/cgi/search.pl?search_terms=${encodeURIComponent(
      query
    )}&search_simple=1&action=process&json=1&page_size=${limit}&fields=code,product_name,brands,serving_quantity,serving_size,nutriments`,
    { headers: { "User-Agent": "NutriTrack - nutritrack.app" } }
  );
  if (!res.ok) return [];
  const data = await res.json();
